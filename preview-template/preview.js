        ...
        selectedNode() {
            ...
            if (node) {
                ...
                for (let component of node._components) {
                    ...
                    if (!schema){
                        for (let i in NEX_CONFIG.componentsSchema){
                            let v = NEX_CONFIG.componentsSchema[i];
                            if ((v.include && component.__classname__.indexOf(v.include)>=0) ||
                                (v.match && v.match instanceof RegExp && v.match.test(component.__classname__)) ){
                                NEX_CONFIG.componentsSchema[component.__classname__] = Object.assign({},v);
                                schema = NEX_CONFIG.componentsSchema[component.__classname__];
                                schema.key = component.__classname__;
                                break;
                            }
                        }
                    }
